...
export const getAllFeeStructures = async (req: Request, res: Response) => {
  try {
    const feeStructures = await prisma.feeStructure.findMany({
      include: { program: { select: { title: true } } }, // <-- Changed from 'course'
...
export const addFeeStructure = async (req: Request, res: Response) => {
  const { name, amount, programId } = req.body; // <-- Changed from 'courseId'
  if (!name || !amount || !programId) {
    return res.status(400).json({ message: 'All fields are required.' });
  }
  try {
    const newStructure = await prisma.feeStructure.create({
      data: { name, amount: parseFloat(amount), programId }, // <-- Changed from 'courseId'
    });
